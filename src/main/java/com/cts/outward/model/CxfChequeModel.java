...
/**
 * View model for one cheque row needed for CXF/CIBF generation.
 * Maps from cts_cheques (real schema — 28 columns).
 *
 * Key columns used:
 *   id             int8     PK
 *   batch_id       varchar  FK → cts_batches.batch_id
 *   cheque_no      varchar
 *   cheque_id      varchar  (instrument serial, used as ItemSeqNo)
 *   account_no     varchar
 *   amount         numeric
 *   cheque_date    varchar
 *   drawer_name    varchar
 *   payee_name     varchar
 *   payee_account_no varchar
 *   sort_code      varchar  (drawee bank routing)
 *   base_no        varchar  (presenting bank routing)
 *   transaction_code varchar
 *   iqa_status     varchar
 *   front_image    bytea    ← images stored in DB
 *   rear_image     bytea    ← images stored in DB
 *   ver_status     varchar
 */
public class CxfChequeModel {
