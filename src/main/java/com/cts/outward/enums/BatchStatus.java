 ...
 *
 * <p>
 * Each constant encapsulates the exact DB string ({@link #db()}) and the
 * human-readable UI label ({@link #getLabel()}), keeping status strings in one
 * place and eliminating scattered raw-string literals across the codebase.
 *
 * <h3>Typical usage</h3>
 * 
 * <pre>
...
public enum BatchStatus {
	...
	/**
	 * Initial state — batch header created in Step 1 modal but ZIP not yet
	 * uploaded.
	 *
	 * <p>
	 * <b>Set by:</b> {@code BatchServiceImpl.createBatch()} when the Maker fills in
	 * the Create Batch form (Step 1) before uploading a ZIP. Also set by
	 * {@code ZipParser.buildBatchEntity()} for direct-upload (Path A) before the
	 * entity is persisted.
	 *
	 * <p>
	 * <b>UI behaviour:</b> Batch row shows a "Draft" chip in the listing grid. The
	 * scan modal (Step 2) remains available. Batch can still be discarded.
	 *
	 * <p>
	 * <b>DB value:</b> {@code "Draft"}
	 */
	DRAFT("Draft", "Draft"),

	/**
	 * Maker has uploaded the ZIP — some cheques are Ready, others still Pending.
	 * The Maker is actively reviewing and saving cheques in
	 * {@code BatchDetailComposer}.
	 *
	 * <p>
	 * <b>Set by:</b> {@code ZipImportServiceImpl.importZip()} via
	 * {@code BatchDAOImpl.updateBatchActualCounts()} immediately after the ZIP is
	 * parsed and cheques are saved to {@code cts_cheques}.
	 *
	 * <p>
	 * <b>UI behaviour:</b> Batch detail page is editable. "Save Batch" button
	 * becomes available once all cheques are marked Ready. Maker cannot submit
	 * until every cheque has been actioned.
	 *
	 * <p>
	 * <b>DB value:</b> {@code "VerificationInProgressAtMaker"} (legacy name from
	 * ClearPay spec — do not rename)
	 */
	PENDING("VerificationInProgressAtMaker", "Pending"),

	/**
	 * All cheques in the batch have been reviewed and saved by the Maker. The batch
	 * is now locked read-only for the Maker and sits in the Verifier's queue.
	 *
	 * <p>
	 * <b>Set by:</b> {@code BatchServiceImpl.saveBatch()} when the Maker clicks
	 * "Save Batch" and all cheques carry status = READY.
	 *
	 * <p>
	 * <b>UI behaviour:</b> Batch detail page becomes read-only for the Maker.
	 * Edit/delete buttons are hidden. Verifier sees the batch in their queue.
	 *
	 * <p>
	 * <b>DB value:</b> {@code "ReadyForVerification"} (replaced earlier value
	 * "SubmittedByMaker" — DB migration ran June 2026)
	 */
	 ...
	 *
	 * <p>
	 * <b>Set by:</b> {@code VerificationOneComposer} or
	 * {@code VerificationTwoComposer} when the verifier opens the batch detail page
	 * (prevents two verifiers acting on the same batch simultaneously).
	 *
	 * <p>
	 * <b>UI behaviour:</b> Batch appears as "In Progress" in the Verifier listing
	 * grid. Other verifiers see it as locked.
	 *
	 * <p>
	 * <b>DB value:</b> {@code "VerificationInProgress"}
	 */
	VERIFICATION_IN_PROGRESS("VerificationInProgress", "Verification In Progress"),

	/**
	 * All cheques in the batch have been actioned (Accepted/Rejected/Referred) by
	 * the required Verifier level(s). Batch is ready for CXF file generation.
	 *
	 * <p>
	 * <b>Set by:</b> {@code VerificationOneComposer} /
	 * {@code VerificationTwoComposer} when the last pending cheque in the batch is
	 * actioned.
	 *
	 * <p>
	 * <b>UI behaviour:</b> "Generate CXF" button becomes visible in
	 * {@code BatchDetailComposer}. Batch cannot be re-opened for editing.
	 *
	 * <p>
	 * <b>DB value:</b> {@code "Verified"}
	 */
	 ...
	 *
	 * <p>
	 * <b>Set by:</b> {@code CBSService.generateCxfFile()} on success, communicated
	 * to the composer via {@code CxfFileResult.ok(...)}.
	 *
	 * <p>
	 * <b>UI behaviour:</b> "Download CXF" button appears. "Dispatch" button becomes
	 * available. Batch detail shows generation timestamp.
	 *
	 * <p>
	 * <b>DB value:</b> {@code "CxfGenerated"}
	 */
	CXF_CIBF_GENERATED("CXF_CIBF_GENERATED", "CXF Generated"),

	/**
	 * CXF file has been submitted to the ClearPay clearing network. This is the
	 * terminal state — no further transitions are possible.
	 *
	 * <p>
	 * <b>Set by:</b> Dispatch action (post-CXF generation step).
	 *
	 * <p>
	 * <b>UI behaviour:</b> All action buttons hidden. Batch appears as "Dispatched"
	 * with a green terminal chip in all listing grids.
	 *
	 * <p>
	 * <b>DB value:</b> {@code "Dispatched"}
	 */
	...
	/**
	 * Exact string stored in {@code cts_batches.status} column. Must never be
	 * changed after data is in production — DB rows hold this value.
	 */
	private final String dbValue;

	/**
	 * Human-readable text for UI display (status chips, dropdowns, dialogs). Safe
	 * to rename without DB migration impact.
	 */
	...
	/**
	 * Returns the human-readable label for UI display in status chips, dropdowns,
	 ...
	 *
	 * <p>
	 * Case-insensitive and trims whitespace — safe to call directly with raw values
	 * from HQL query results or JSON deserialization.
	 *
	 * <p>
	 * Returns {@link #DRAFT} as a safe default when the input is {@code null},
	 * blank, or does not match any known status. Callers that need to distinguish
	 * "unknown" from "draft" should check the raw value first.
	 *
	 * <pre>
	 ...
	 *
	 * <p>
	 * <b>Called by:</b> {@code MyBatchesComposer} (status chip rendering),
	 * {@code BatchDetailComposer} (button visibility logic), {@code BatchDAOImpl}
	 * (result mapping).
	 *
	 ...
	 */
	public static BatchStatus fromDb(String raw) {
