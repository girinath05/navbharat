...
/**
 * ZK SelectorComposer for {@code pending-batches.zul}.
 *
 * <p>Displays only {@code VerificationInProgressAtMaker} (Pending) batches —
 * batches where the ZIP has been uploaded and the Maker is actively entering
 * cheque data but has not yet clicked "Save Batch".
 *
 * <h3>Status scope</h3>
 * <ul>
 *   <li>Shows: {@link BatchStatus#PENDING} ({@code "VerificationInProgressAtMaker"}) only.</li>
 *   <li>No "Create Batch" button — creation happens in Scan Module (Draft state).</li>
 * </ul>
 *
 * <h3>Architecture</h3>
 * <pre>
 *   pending-batches.zul
 *       └── PendingBatchesComposer
 ...
 * @see BatchService
 */
public class PendingBatchesComposer extends SelectorComposer<Component> {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(PendingBatchesComposer.class.getName());

    private static final int PAGE_SIZE = 5;

    /** Hardcoded status — this page shows Pending only. */
    private static final String STATUS_FILTER = BatchStatus.PENDING.db(); // "VerificationInProgressAtMaker"
    ...
    private void appendRow(BatchEntity b) {
        ...
        // Col 5: Status chip — always "Pending" on this page
        Listcell stCell = new Listcell();
        Label stLbl = new Label("Pending");
        stLbl.setSclass("chip ch-progress"); // amber/yellow chip for Pending
        stCell.appendChild(stLbl);
        row.appendChild(stCell);

        // Col 6: Action — View & Edit only (no Save Batch; that lives in batch-detail)
    ...
    private void openBatch(String batchId) {
        ...
        Sessions.getCurrent().setAttribute("batchDetailBackPage", "/zul/outward/pending-batches.zul");
